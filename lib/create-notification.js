  ...
  try {
    ...
    switch (eventType) {
      case 'lab-appointment': {
        ...
        const messageForDoctor = `You have scheduled a lab appointment for ${receiver.name} at ${
          lab.labName
        } on ${lab.bookingDateTime.toUTCString()}.`;

        const messageForPatient = `Your lab appointment has been scheduled by ${sender.name} at ${
          lab.labName
        } on ${lab.bookingDateTime.toUTCString()}.`;
