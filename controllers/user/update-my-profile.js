...
async function handlePatientUpdate(data, existingUser) {
    ...
    if (data.patientDetails.idVerification) {
        data.patientDetails.idVerification = await handleBase64Upload(data.patientDetails.idVerification, randomBytes(16).toString('hex'));
    }
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        // await existingUser.updateOne(data.userDetails, { session });
        await UserModel.findOneAndUpdate({ _id: existingUser._id }, data.userDetails, { session });
        const patient = await PatientModel.findOneAndUpdate({ user: existingUser._id }, data.patientDetails, { new: true, session });
        if (!patient) {
            throw new HttpError({
        ...
        await PatientMedicalHistoryModel.findOneAndUpdate({ patient: patient._id }, { ...data.medicalHistory, disease: disease }, { new: true, session });
...
async function handleDoctorUpdate(data, existingUser) {
    ...
    try {
        session.startTransaction();
        await UserModel.findOneAndUpdate({ _id: existingUser._id }, data.userDetails, { session });
        const doctor = await DoctorModel.findOneAndUpdate({ user: existingUser._id }, data.doctorDetails, { new: true, session });
        if (doctor) {
...
async function handleDonorAcquirerUpdate(data, existingUser) {
    const session = await mongoose.startSession();
    try {
        session.startTransaction();
        await UserModel.findOneAndUpdate({ _id: existingUser._id }, data.userDetails, { session });
        await DonorAcquirerModel.findOneAndUpdate({ user: existingUser._id }, data.donorAcquirerDetails, { new: true, session });
        await session.commitTransaction();
    }
    catch (error) {
        await session.abortTransaction();
